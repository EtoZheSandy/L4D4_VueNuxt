...
export default {
  ...
  methods: {
    async fetchPlayerProfile() {
      try {
        const response = await fetch(`${this.config.public.apiBase}/v1/player_info/${this.steamid}`);
        if (response.ok) {
    ...
    async fetchPlayerSteam() {
      try {
        const response = await fetch(`${this.config.public.apiBase}/v1/get_steam/${this.steamid}`);
        if (response.ok) {
    ...
    async fetchPlayerBan() {
      try {
        const response = await fetch(`${this.config.public.apiBase}/v1/bans/${this.steamid}`);
        if (response.ok) {
          const data = await response.json();
          this.BanData = data[0];
        } else {
    ...
    async AllTimeScore() {
      try {
        const response = await fetch(`${this.config.public.apiBase}/v1/alltimescore/${this.steamid}`);
        if (response.ok) {
          const data = await response.json();
